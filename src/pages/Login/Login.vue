...
<script>
  ...
  export default {
    ...
    data () {
      return {
        loginWay: true, // true短信登陆， false代表密码登陆
        phone: '', //手机号码
        code: '', //短信验证码
        codeTime: 0, // 计时的时间
    ...
    methods: {
      ...
      login () {
        ...
        if (this.loginWay) { // 短信登陆
          ...
          if (!this.rightPhone) {
            // 手机号码不正确
            this.showAlerts('手机号码不正确');
          } else if (!/^\d{6}$/.test(code)) {
          // 验证码必须是六位数字
            this.showAlerts('请输入正确的验证码');
          }
        } else { // 密码登陆
          const {name, pwd, captcha} = this;
          if (!this.name) {
            // 用户名必须指定
            this.showAlerts('用户名必须指定');
          } else if (!this.pwd) {
            // 密码必须指定
